...
async function emptyBucket(bucketName: string) {
  try {
    ...
    if (listError) {
      ...
      console.error(`   ❌ Error listing files in bucket '${bucketName}':`, listError.message);
      return false;
    }
    ...
    console.error(`   ❌ Unexpected error emptying bucket '${bucketName}':`, error);
    return false;
  }
}
...
async function deleteBucket(bucketName: string) {
  try {
    ...
    console.error(`   ❌ Unexpected error deleting bucket '${bucketName}':`, error);
    return false;
  }
}
...
async function verifyBucketDeleted(bucketName: string) {
  try {
    ...
    console.error(`   ❌ Unexpected error verifying bucket deletion '${bucketName}':`, error);
    return false;
  }
}
...
async function main() {
  ...
  console.log('');
  console.log('⚠️  WARNING: This will permanently delete all storage buckets and their contents!');
  console.log('');
  ...
  console.log('================');
  console.log(`✅ Successfully deleted: ${successCount}/${totalBuckets} buckets`);
  console.log('');
  ...
  if (successCount === totalBuckets) {
    ...
    console.log('⚠️  Some buckets failed to delete. Please check the errors above.');
    process.exit(1);
  }
}
