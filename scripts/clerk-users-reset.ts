...
async function main() {
  ...
  try {
    ...
    for (const user of allUsers) {
      try {
        log.step(
          `Deleting user: ${user.emailAddresses[0]?.emailAddress ?? user.id} (${deletedCount + 1}/${allUsers.length})`,
        );
    ...
    try {
      ...
      if (orphanedPlayers.length > 0) {
        ...
        log.success(
          `Cleaned up ${orphanedPlayers.length} orphaned player records and all related data`,
        );
      }
      ...
      if (orphanedCoaches.length > 0) {
        ...
        log.success(
          `Cleaned up ${orphanedCoaches.length} orphaned coach records and all related data`,
        );
      }
      ...
      if (orphanedLeagueAdmins.length > 0) {
        ...
        log.success(
          `Cleaned up ${orphanedLeagueAdmins.length} orphaned league administrator records and all related data`,
        );
      }

      if (
        orphanedPlayers.length === 0 &&
        orphanedCoaches.length === 0 &&
        orphanedLeagueAdmins.length === 0
      ) {
        log.info("No orphaned database records found");
      }
    } catch (dbError) {
      log.warning(
        `Database cleanup failed: ${dbError instanceof Error ? dbError.message : "Unknown error"}`,
      );
    }
    ...
    log.info(
      "Note: Database webhooks should have automatically cleaned up associated records",
    );
    log.info(
      'Consider running "npm run db:reset:script" to ensure a completely clean state',
    );
  } catch (error) {
    log.error(
