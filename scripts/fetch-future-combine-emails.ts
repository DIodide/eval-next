...
async function main() {
  ...
  try {
    ...
    futureCombines.forEach((combine, index) => {
      log.data(`${index + 1}. ${combine.title} - ${combine.date.toISOString()} @ ${combine.location} (${combine.registered_spots}/${combine.max_spots} spots)`);
    });
    ...
    log.success(`Found ${registrations.length} registrations for future combines`);

    // Extract unique emails
    const uniqueEmails = Array.from(new Set(registrations.map(r => r.player.email)));
    
    console.log('\n' + '='.repeat(80));
    log.success(`SUMMARY: ${uniqueEmails.length} unique player emails registered for future combines`);
    ...
    for (const combine of futureCombines) {
      const combineRegistrations = registrations.filter(r => r.combine_id === combine.id);
      ...
      if (combineRegistrations.length > 0) {
        ...
        combineRegistrations.forEach((reg, index) => {
          ...
          const displayName = player.username ?? `${player.first_name} ${player.last_name}`;
    ...
    log.error(`An error occurred: ${error instanceof Error ? error.message : 'Unknown error'}`);
    console.error(error);
    process.exit(1);
  } finally {
