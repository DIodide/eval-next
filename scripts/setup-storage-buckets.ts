...
async function createStorageBucket(bucketName: string, options: BucketOptions) {
  try {
    ...
    console.error(
      `   ❌ Unexpected error creating bucket '${bucketName}':`,
      error,
    );
    return false;
  }
...
async function createRLSPolicy(bucketName: string) {
  try {
    ...
    console.error(
      `   ❌ Error setting up RLS policies for ${bucketName}:`,
      error,
    );
    return false;
  }
...
async function verifyBucketExists(bucketName: string) {
  try {
    ...
    if (error) {
      console.log(
        `   ❌ Bucket '${bucketName}' verification failed:`,
        error.message,
      );
      return false;
    }
    ...
    console.error(
      `   ❌ Unexpected error verifying bucket '${bucketName}':`,
      error,
    );
    return false;
  }
...
async function main() {
  ...
  console.log("");
  console.log(
    "⚠️  IMPORTANT: You need to manually set up RLS policies for security.",
  );
  console.log("");
  ...
  console.log("================");
  console.log(
    `✅ Successfully created/verified: ${successCount}/${totalBuckets} buckets`,
  );
  console.log("");
