...
export function useGameSelection(
  ...
  const availableGames = GAME_CONFIGS.filter(
    (game) => !allowedGames || allowedGames.includes(game.id),
  );
    ...
    (event: React.KeyboardEvent) => {
      ...
      switch (event.key) {
        case "ArrowLeft":
          ...
          const prevIndex =
            currentIndex > 0 ? currentIndex - 1 : availableGames.length - 1;
        ...
        case "ArrowRight":
          event.preventDefault();
          const nextIndex =
            currentIndex < availableGames.length - 1 ? currentIndex + 1 : 0;
...
export function useGameFilters(
  ...
  const filterGames = useCallback(
    (allGames: GameId[], connections: Record<GameId, boolean>) => {
      return allGames.filter((gameId) => {
        ...
        if (
          ...
          connections[gameId] !== filters.connected
        ) {
          return false;
        }
        ...
        if (
          ...
          gameConfig.enabled !== filters.enabled
        ) {
          return false;
        }

        if (filters.supported !== undefined) {
          const isSupported = ["valorant", "rocket-league", "smash"].includes(
            gameId,
          );
