...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border-2 p-4 pr-8 shadow-xl backdrop-blur-sm transition-all duration-300 data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border-cyan-500/30 bg-gray-800/95 text-white shadow-cyan-500/20",
        destructive: "border-red-500/50 bg-red-900/90 text-white shadow-red-500/30",
        success: "border-green-500/50 bg-green-900/90 text-white shadow-green-500/30",
        warning: "border-yellow-500/50 bg-yellow-900/90 text-white shadow-yellow-500/30",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md border border-gray-600 bg-gray-700/80 px-3 text-sm font-medium text-white transition-colors hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:ring-offset-gray-800 disabled:pointer-events-none disabled:opacity-50",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-md p-1 text-white/70 opacity-0 transition-all hover:text-white hover:bg-white/10 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-cyan-500 group-hover:opacity-100",
      className
    )}
    toast-close=""
    {...props}
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    className={cn("text-sm font-semibold font-orbitron", className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    className={cn("text-sm opacity-90 font-rajdhani", className)}
    {...props}
  />
))
...
const ToastIcon = ({ variant }: { variant?: "default" | "destructive" | "success" | "warning" | "info" }) => {
  ...
  switch (variant) {
    case "success":
      return <CheckCircle {...iconProps} className="h-5 w-5 shrink-0 text-green-400" />
    case "destructive":
      return <AlertCircle {...iconProps} className="h-5 w-5 shrink-0 text-red-400" />
    case "warning":
      return <AlertTriangle {...iconProps} className="h-5 w-5 shrink-0 text-yellow-400" />
    case "info":
