...
export function NewConversationDialog({
  ...
  const { data: players, isLoading: playersLoading } =
    api.messages.getAvailablePlayers.useQuery(
      { search: deferredSearchQuery, limit: 50 },
  ...
  const contacts =
    role === "coach"
      ? (players ?? []).map((p) => ({
          ...
          avatar: p.avatar,
          subtitle: [
            p.mainGame ? getGameIcon(p.mainGame) + " " + p.mainGame : null,
            p.school,
          ]
            .filter(Boolean)
            .join(" · "),
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[80vh] max-w-2xl overflow-y-auto border-gray-700/50 bg-gradient-to-br from-gray-800/95 to-gray-900/95 text-white backdrop-blur-sm">
        ...
        <div className="space-y-6">
          {role === "player" && !canStartConversation ? (
            <>
              <MessagingPaywall
                compact
                title="Free Quota Reached"
                description={
                  playerQuota?.monthlyConversationStartLimit
                    ? `You have used all ${playerQuota.monthlyConversationStartLimit} free coach intros for this month. Existing threads still work normally.`
                    : "You have used your free monthly coach intros."
                }
              />
            ...
            <>
              <div>
                ...
                {isLoading ? (
                  ...
                  <div className="rounded-lg border border-dashed border-gray-700/50 px-4 py-8 text-center text-sm text-gray-400">
                    No matches found.
                  </div>
                ) : (
