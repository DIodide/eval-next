...
export function getUserRole(user: ClerkUser): UserRole {
  ...
  if (
    publicUserType === "player" ||
    publicUserType === "coach" ||
    publicUserType === "league"
  ) {
  ...
  if (
    unsafeUserType === "player" ||
    unsafeUserType === "coach" ||
    unsafeUserType === "league"
  ) {
...
export function isCoachOnboarded(user: ClerkUser): boolean {
  ...
  const publicOnboarded = user?.publicMetadata?.onboarded as
    | boolean
    | undefined;
  if (publicOnboarded === true) return true;

  // Fallback to unsafeMetadata for compatibility
  const unsafeOnboarded = user?.unsafeMetadata?.onboarded as
    | boolean
    | undefined;
...
export function canAccessCoachFeaturesFromStatus(
  onboardingStatus: CoachOnboardingStatus,
): boolean {
...
export function getOnboardingMessage(
  ...
  if (
    !onboardingStatus.hasSchoolAssociation &&
