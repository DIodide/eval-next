...
export const UPLOAD_CONSTRAINTS = {
  ...
  RECOMMENDED_SIZES: {
    ...
    BANNER: {
      width: 1200,
      height: 300,
      description: "Banner image (1200x300px recommended)",
    },
    AVATAR: {
      width: 200,
      height: 200,
      description: "Profile picture (200x200px recommended)",
    },
    THUMBNAIL: {
      width: 300,
      height: 200,
...
export function validateFile(file: File): { isValid: boolean; error?: string } {
  ...
  if (
    !UPLOAD_CONSTRAINTS.ALLOWED_TYPES.includes(
      file.type as (typeof UPLOAD_CONSTRAINTS.ALLOWED_TYPES)[number],
    )
  ) {
    return {
      isValid: false,
...
export async function uploadFile(
  ...
  try {
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from(STORAGE_BUCKETS[bucket]).getPublicUrl(filePath);
