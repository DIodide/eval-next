...
import type { FilterStatus, MessageItem, PlayerConversationDetail, PlayerConversation } from "@/components/messaging/types";
import { formatLastSeen, getInitials } from "@/components/messaging/utils";
import {
  useConversations,
  useSendMessage,
  useMarkAsRead,
...
export default function PlayerMessagesPage() {
  ...
  const listQuery = useConversations("player", { search, filter });
  const detailQuery = api.messages.getPlayerConversation.useQuery(
    { conversationId: selectedId! },
    { enabled: !!selectedId, refetchInterval: 2_000 },
  );

  const sendMutation = useSendMessage("player", selectedId);
  const markReadMutation = useMarkAsRead("player");
  const starMutation = useToggleStar("player");

  const sendNewMutation = api.messages.sendPlayerMessage.useMutation({
    onSuccess: (data) => {
  ...
  const conversations = (listQuery.data?.conversations ?? []) as PlayerConversation[];
  const detail = detailQuery.data as PlayerConversationDetail | undefined;
  ...
  return (
    <div className="flex h-full flex-col gap-4 p-4 lg:p-6">
                ...
                >
                  <div className="relative flex-shrink-0">
                    <Avatar className="h-10 w-10 border border-gray-700">
                      <AvatarImage src={conv.coach.avatar ?? undefined} />
                      <AvatarFallback className="bg-gray-700 text-xs text-white">
                        {getInitials(conv.coach.name)}
                      </AvatarFallback>
                    </Avatar>
                    {conv.unreadCount > 0 && (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {conv.coach.school && (
                      <p className="truncate text-[11px] text-gray-500">{conv.coach.school}</p>
                    )}
                    {conv.lastMessage && (
                      <p className="truncate text-xs text-gray-400">
                        {conv.lastMessage.senderType === "PLAYER" && (
        ...
        <div className="flex min-w-0 flex-1 flex-col bg-gray-900/40">
          {selectedId && detail ? (
            <>
              ...
              <div className="flex items-center justify-between border-b border-gray-700/50 px-5 py-3">
                <div className="flex items-center gap-3">
                  <Avatar className="h-10 w-10 border-2 border-cyan-500/30">
                    <AvatarImage src={detail.coach.avatar ?? undefined} />
                    <AvatarFallback className="bg-gray-700 text-white">
                      {getInitials(detail.coach.name)}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    ...
                    {detail.coach.school && (
                      <p className="text-xs text-gray-400">{detail.coach.school}</p>
                    )}
                  </div>
                </div>
              ...
              <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-4">
                {detailQuery.isLoading ? (
                  ...
                  <div className="space-y-3">
                    {messages.map((msg) => {
                      const isSelf = msg.senderType === "PLAYER";
                      return (
              ...
              {/* Input area — players can always reply to existing conversations */}
      ...
      <NewConversationDialog
        ...
        onSend={(coachId, content) => sendNewMutation.mutate({ coachId, content })}
