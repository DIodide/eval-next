  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        ...
        <Skeleton className="h-10 w-32 bg-gray-700 mt-4 sm:mt-0" />
      </div>
...
function SchoolTypeIcon({ type }: { type: string }) {
  switch (type) {
    ...
    case 'COLLEGE':
    case 'UNIVERSITY':
      return <Building2Icon className="w-4 h-4" />;
    default:
      return <Building2Icon className="w-4 h-4" />;
  }
}
...
function SchoolTypeBadge({ type }: { type: string }) {
  ...
  return (
    <Badge className={`${colors[type as keyof typeof colors] || 'bg-gray-600 text-gray-100'} border-0`}>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md bg-gray-900 border-gray-700 text-white">
        <DialogHeader>
          <DialogTitle className="font-orbitron text-xl flex items-center gap-2 text-red-400">
        ...
        <div className="py-4">
          <p className="text-gray-300">
            Are you sure you want to remove <span className="font-semibold text-white">{schoolName}</span> from your league?
          </p>
...
function AddSchoolDialog() {
  ...
  // Search schools
  const { data: searchResults, isLoading: isSearching } = api.leagueAdminProfile.searchAvailableSchools.useQuery({
    search: searchQuery,
    limit: 10,
    ...(selectedType !== "all" && { type: selectedType as "HIGH_SCHOOL" | "COLLEGE" | "UNIVERSITY" }),
  ...
  const addSchoolMutation = api.leagueAdminProfile.addSchoolToLeague.useMutation({
    onSuccess: (data) => {
            ...
            <div className="space-y-4">
              ...
              {searchResults && searchResults.length > 0 && (
                <div className="space-y-3">
                  <h3 className="font-medium text-gray-300">Search Results ({searchResults.length})</h3>
              ...
              {searchQuery.length < 2 && (
                <div className="text-center py-8 text-gray-400">
                  ...
                  <p className="text-sm mt-1">Enter at least 2 characters to begin searching</p>
                </div>
              )}
            </div>
          </div>
...
export default function LeagueSchoolsPage() {
  ...
  // Get league schools
  const { data: leagueSchools, isLoading } = api.leagueAdminProfile.getLeagueSchools.useQuery();

  // Remove school mutation
  const removeSchoolMutation = api.leagueAdminProfile.removeSchoolFromLeague.useMutation({
    onSuccess: (data) => {
  ...
  const handleConfirmRemove = () => {
    removeSchoolMutation.mutate({ league_school_id: confirmDialog.leagueSchoolId });
  };
  ...
  const totalPlayers = leagueSchools?.reduce((sum, ls) => sum + ls.school._count.players, 0) ?? 0;
  const totalCoaches = leagueSchools?.reduce((sum, ls) => sum + ls.school._count.coaches, 0) ?? 0;
          ...
          {totalSchools > 0 ? (
            <div className="space-y-4">
              {leagueSchools?.map((leagueSchool) => (
                <div
                  key={leagueSchool.id}
                  className="flex items-center justify-between p-4 bg-gray-800 rounded-lg border border-gray-700 hover:border-purple-500 transition-colors"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 text-gray-400 mb-2">
                        ...
                        <span>{leagueSchool.school.location}, {leagueSchool.school.state}</span>
                        {leagueSchool.school.region && (
                          <span className="text-gray-500">• {leagueSchool.school.region}</span>
                        )}
                      </div>
                      <div className="flex items-center gap-6 text-sm text-gray-400">
                        <div className="flex items-center gap-1">
                          <Users className="w-4 h-4" />
                          <span>{leagueSchool.school._count.players} players</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <GraduationCap className="w-4 h-4" />
                          <span>{leagueSchool.school._count.coaches} coaches</span>
                        </div>
                        <div className="flex items-center gap-1">
                        ...
                        <div className="text-gray-500">
                          Joined {new Date(leagueSchool.joined_at).toLocaleDateString()}
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <Button
                      onClick={() => handleRemoveSchool(leagueSchool.id, leagueSchool.school.name)}
            ...
            <div className="text-center py-12 text-gray-400">
              <Building2Icon className="w-16 h-16 mx-auto mb-4 text-gray-600" />
              <h3 className="font-medium text-lg mb-2">No schools added yet</h3>
              <p className="text-sm mb-6 max-w-md mx-auto">
                Get started by adding schools to your league. Schools can participate with their teams and players in your competitions.
              </p>
              <AddSchoolDialog />
            </div>
          )}
