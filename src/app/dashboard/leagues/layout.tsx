...
import { isLeagueOnboarded } from "@/lib/permissions";
import { api } from "@/trpc/react";

// Define protected routes that require onboarding
const protectedRoutes = [
  "/dashboard/leagues/tournaments", 
  "/dashboard/leagues/teams",
  "/dashboard/leagues/matches"
];
...
export default function LeaguesDashboardLayout({
  ...
  // Fetch league organization profile
  const { data: profile } = api.leagueProfile.getProfile.useQuery(undefined, {
    enabled: isLoaded && !!user,
  });

  // Check if user is a league organization
  if (isLoaded && user) {
  ...
  useEffect(() => {
    if (isLoaded && user) {
      const isOnboarded = isLeagueOnboarded(user);
  ...
  const isOnboarded = user ? isLeagueOnboarded(user) : false;
  ...
  const sidebarItems = [
    ...
    {
      title: "Tournaments",
      href: "/dashboard/leagues/tournaments",
      icon: TrophyIcon,
    ...
    {
      title: "Matches",
      href: "/dashboard/leagues/matches",
      icon: CalendarIcon,
      requiresOnboarding: true,
      enabled: true,
    },
