...
import { CheckCircleIcon, PlusIcon, SendIcon, X, BuildingIcon, ArrowRightIcon } from "lucide-react";
import { api } from "@/trpc/react";
import { useToast } from "@/hooks/use-toast";
import { MultiSelectGames, type CustomGame } from "@/components/ui/multi-select-games";
...
export function LeagueAssociationRequestForm({ onClose }: LeagueAssociationRequestFormProps) {
  ...
  const [isNewLeagueRequest, setIsNewLeagueRequest] = useState<boolean | null>(null);
  ...
  const [newLeagueData, setNewLeagueData] = useState({
    ...
    proposed_region: "",
    proposed_state: "",
    proposed_tier: "" as "ELITE" | "PROFESSIONAL" | "COMPETITIVE" | "DEVELOPMENTAL" | "",
    proposed_season: "",
    proposed_format: "",
  ...
  const submitRequest = api.leagueAdminProfile.submitLeagueAssociationRequest.useMutation({
    onSuccess: () => {
      setSuccess(true);
      setError(null);
      toast({
        title: "Request Submitted",
        description: "Your league association request has been submitted successfully. We'll review it and get back to you soon.",
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNewLeagueRequest && selectedGameIds.length === 0 && customGames.length === 0) {
    ...
    if (isNewLeagueRequest) {
      submissionData = {
        ...
        proposed_game_ids: selectedGameIds.length > 0 ? selectedGameIds : undefined,
        ...
        proposed_tier: newLeagueData.proposed_tier as "ELITE" | "PROFESSIONAL" | "COMPETITIVE" | "DEVELOPMENTAL" | undefined,
        proposed_season: newLeagueData.proposed_season,
        proposed_format: newLeagueData.proposed_format || undefined,
        proposed_founded_year: newLeagueData.proposed_founded_year ? parseInt(newLeagueData.proposed_founded_year) : undefined,
      };
    } else {
      submissionData = {
    ...
    return (
      <Card className="bg-gray-800 border-gray-700 shadow-2xl">
        <CardHeader className="text-center pb-6">
          ...
          <CardTitle className="text-white font-orbitron text-2xl mb-2">League Association</CardTitle>
          <p className="text-gray-400">Choose how you&apos;d like to get started with league management</p>
        </CardHeader>
        <CardContent className="px-8 pb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Existing League Card */}
            <Card 
              className="bg-gray-900/50 border-gray-600 hover:border-purple-500/50 transition-all duration-300 group cursor-pointer relative overflow-hidden"
              ...
              <CardHeader className="relative z-10">
                <CardTitle className="text-white flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-400 font-normal">Connect with your league</div>
                  </div>
                </CardTitle>
              </CardHeader>
              <CardContent className="relative z-10">
                <p className="text-gray-400 mb-6 leading-relaxed">
                  If your league already exists on Eval, request administrator access to start managing players, teams, and tournaments.
                </p>
            ...
            {/* New League Card */}
            <Card 
              className="bg-gray-900/50 border-gray-600 hover:border-cyan-500/50 transition-all duration-300 group cursor-pointer relative overflow-hidden"
              ...
              <CardHeader className="relative z-10">
                <CardTitle className="text-white flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-400 font-normal">Start from scratch</div>
                  </div>
                </CardTitle>
              </CardHeader>
              <CardContent className="relative z-10">
                <p className="text-gray-400 mb-6 leading-relaxed">
                  Don&apos;t see your league? Request to create a new league on Eval with custom games, rules, and branding.
                </p>
          ...
          <div className="mt-8 text-center">
            <p className="text-xs text-gray-500">
              For additional support, please contact us at <a href="mailto:support@evalgaming.com" className="text-cyan-400 hover:text-cyan-300">support@evalgaming.com</a>.
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-white font-orbitron">
              {isNewLeagueRequest ? "Create New League" : "Join Existing League"}
            </CardTitle>
          </div>
          {onClose && (
            <Button
          ...
          {isNewLeagueRequest ? (
            ...
            <div className="space-y-4">
              <div className="bg-cyan-900/20 border border-cyan-500 rounded-lg p-4">
                <h4 className="text-cyan-400 font-medium mb-2">Creating a New League</h4>
                <p className="text-gray-300 text-sm">
                  You&apos;re requesting to create a brand new league. Please provide detailed information 
                  about your proposed league. Our team will review your request and help you get started.
                </p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="proposed_league_name" className="text-white">League Name *</Label>
                  <Input
                    id="proposed_league_name"
                    placeholder="Enter the full league name"
                    value={newLeagueData.proposed_league_name}
                    onChange={(e) => setNewLeagueData({...newLeagueData, proposed_league_name: e.target.value})}
                ...
                <div className="space-y-2">
                  <Label htmlFor="proposed_league_short_name" className="text-white">Short Name *</Label>
                  <Input
                    id="proposed_league_short_name"
                    placeholder="e.g., NECL, VUOL"
                    value={newLeagueData.proposed_league_short_name}
                    onChange={(e) => setNewLeagueData({...newLeagueData, proposed_league_short_name: e.target.value})}
                    className="bg-gray-800 border-gray-700 text-white"
                    required
                  />
                </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="proposed_league_description" className="text-white">League Description *</Label>
                <Textarea
                  ...
                  onChange={(e) => setNewLeagueData({...newLeagueData, proposed_league_description: e.target.value})}
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-400">
                  Select from Eval&apos;s supported games or add custom games that your league will host.
                </p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="proposed_region" className="text-white">Region *</Label>
                  <Input
                    id="proposed_region"
                    placeholder="e.g., North America, Europe"
                    value={newLeagueData.proposed_region}
                    onChange={(e) => setNewLeagueData({...newLeagueData, proposed_region: e.target.value})}
                    className="bg-gray-800 border-gray-700 text-white"
                    required
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label htmlFor="proposed_state" className="text-white">State/Province</Label>
                  <Input
                    id="proposed_state"
                    placeholder="e.g., California, Ontario"
                    value={newLeagueData.proposed_state}
                    onChange={(e) => setNewLeagueData({...newLeagueData, proposed_state: e.target.value})}
                    className="bg-gray-800 border-gray-700 text-white"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="proposed_tier" className="text-white">Competition Tier</Label>
                  <Select
                    value={newLeagueData.proposed_tier}
                    onValueChange={(value) => setNewLeagueData({...newLeagueData, proposed_tier: value as "ELITE" | "PROFESSIONAL" | "COMPETITIVE" | "DEVELOPMENTAL" | ""})}
                  >
                ...
                <div className="space-y-2">
                  <Label htmlFor="proposed_season" className="text-white">Season *</Label>
                  <Input
                    id="proposed_season"
                    placeholder="e.g., Spring 2024, Fall 2024"
                    value={newLeagueData.proposed_season}
                    onChange={(e) => setNewLeagueData({...newLeagueData, proposed_season: e.target.value})}
                    className="bg-gray-800 border-gray-700 text-white"
                    required
                  />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="proposed_format" className="text-white">Format</Label>
                  <Input
                    id="proposed_format"
                    placeholder="e.g., Round Robin, Tournament"
                    value={newLeagueData.proposed_format}
                    onChange={(e) => setNewLeagueData({...newLeagueData, proposed_format: e.target.value})}
                    className="bg-gray-800 border-gray-700 text-white"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="proposed_founded_year" className="text-white">Founded Year</Label>
                  <Input
                    id="proposed_founded_year"
                    type="number"
                    placeholder="e.g., 2024"
                    value={newLeagueData.proposed_founded_year}
                    onChange={(e) => setNewLeagueData({...newLeagueData, proposed_founded_year: e.target.value})}
                    className="bg-gray-800 border-gray-700 text-white"
                    min="2020"
                    max="2030"
                  />
              ...
              <div className="space-y-2">
                <Label htmlFor="new_league_message" className="text-white">Request Message *</Label>
                <Textarea
                  ...
                  onChange={(e) => setNewLeagueData({...newLeagueData, request_message: e.target.value})}
                  className="bg-gray-800 border-gray-700 text-white min-h-[100px]"
                  maxLength={1000}
                  required
                />
            ...
            <div className="space-y-4">
              <div className="bg-purple-900/20 border border-purple-500 rounded-lg p-4">
                <h4 className="text-purple-400 font-medium mb-2">Join Existing League</h4>
                <p className="text-gray-300 text-sm">
                  Request to become an administrator for an existing league. You&apos;ll need approval 
                  from our team and potentially from current league administrators.
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="league" className="text-white">Select League *</Label>
                <Select value={selectedLeagueId} onValueChange={setSelectedLeagueId} required>
                  <SelectTrigger className="bg-gray-800 border-gray-700 text-white">
              ...
              <div className="space-y-2">
                <Label htmlFor="message" className="text-white">Request Message *</Label>
                <Textarea
                  id="message"
