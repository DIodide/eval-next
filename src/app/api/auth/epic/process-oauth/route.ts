...
export async function POST(req: NextRequest) {
    ...
    if (!hasEpicGamesAccount) {
      return NextResponse.json({ error: 'No Epic Games account found' }, { status: 400 });
    }
    ...
    try {
      ...
      if (!tokenResponse.data || tokenResponse.data.length === 0) {
        ...
        return NextResponse.json({ error: 'No OAuth access token found for Epic Games account' }, { status: 400 });
      }
      ...
      if (!accessToken) {
        ...
        return NextResponse.json({ error: 'Invalid OAuth access token for Epic Games account' }, { status: 400 });
      }
    } catch (tokenError) {
