...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check current state
    const hasEpicAccount = user.externalAccounts.some(
      (acc) =>
    ...
    console.log(
      `[CLEANUP] State: account=${hasEpicAccount}, metadata=${hasEpicMetadata}`,
    );
    ...
    return NextResponse.json({
      success: true,
      cleaned: false,
      message: hasEpicAccount
        ? "User has Epic Games account"
        : "No Epic Games metadata to clean",
    });
  } catch (error) {
    console.error("[CLEANUP] Epic Games error:", error);
    return NextResponse.json(
      { error: "Epic Games cleanup failed" },
      { status: 500 },
    );
  }
}
