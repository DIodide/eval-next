...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check current state
    const hasStartGGAccount = user.externalAccounts.some(acc => 
      acc.provider === 'custom_start_gg'
    );
    const hasStartGGMetadata = !!user.publicMetadata?.start_gg;
    
    console.log(`[CLEANUP] State: account=${hasStartGGAccount}, metadata=${hasStartGGMetadata}`);
    ...
    return NextResponse.json({ 
      success: true, 
      cleaned: false,
      message: hasStartGGAccount ? 'User has start.gg account' : 'No start.gg metadata to clean'
    });
  } catch (error) {
    console.error('[CLEANUP] start.gg error:', error);
    return NextResponse.json({ error: 'start.gg cleanup failed' }, { status: 500 });
  }
} 