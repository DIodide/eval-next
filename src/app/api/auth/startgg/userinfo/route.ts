...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!startggResponse.ok) {
      console.error('start.gg API error:', startggResponse.status, startggResponse.statusText)
      return NextResponse.json(
    ...
    const restResponse = {
      ...
      locale: user.location ? `${user.location.city}, ${user.location.state}, ${user.location.country}` : null,
      picture: user.images?.find((img: StartGGImage) => img.type === 'profile')?.url,
