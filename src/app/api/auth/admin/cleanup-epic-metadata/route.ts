...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      `[ADMIN CLEANUP] Admin ${userId}: cleaning Epic Games metadata for user ${targetUserId}`,
    );

    // Check current state
    const hasEpicAccount = targetUser.externalAccounts.some(
      (acc) =>
    ...
    console.log(
      `[ADMIN CLEANUP] Target user state: account=${hasEpicAccount}, metadata=${hasEpicMetadata}`,
    );

    // Force removal of Epic Games metadata (admin override)
    if (hasEpicMetadata) {
      console.log(
        `[ADMIN CLEANUP] Force removing Epic Games metadata for user ${targetUserId}`,
      );
    ...
    console.log(
      `[ADMIN CLEANUP] No Epic Games metadata to clean for user ${targetUserId}`,
    );
    return NextResponse.json({
      success: true,
      cleaned: false,
    ...
    return NextResponse.json(
      { error: "Admin Epic Games cleanup failed" },
      { status: 500 },
    );
  }
}
