  ...
  try {
    ...
    if (!valorantAccount) {
      return NextResponse.json(
        { error: "No Valorant account found" },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      if (!tokenResponse.data || tokenResponse.data.length === 0) {
        ...
        return NextResponse.json(
          { error: "No OAuth access token found for Valorant account" },
          { status: 400 },
        );
      }
      ...
      if (!accessToken) {
        ...
        return NextResponse.json(
          { error: "Invalid OAuth access token for Valorant account" },
          { status: 400 },
        );
      }
    } catch (tokenError) {
    ...
    console.log(
      `Successfully linked Valorant account for user ${userId}: ${riotData.gameName}#${riotData.tagLine}`,
    );
