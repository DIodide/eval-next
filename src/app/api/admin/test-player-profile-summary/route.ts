...
export async function GET() {
  ...
  try {
    ...
    const functionalityResponse = await fetch(
      "http://localhost:3000/api/test-player-profile",
    );
    ...
    const validationResponse = await fetch(
      "http://localhost:3000/api/test-player-profile-validation",
    );
    ...
    const totalPassed =
      functionalityData.summary.passed + validationData.summary.passed;
    const totalFailed =
      functionalityData.summary.failed + validationData.summary.failed;
    const totalTests =
      functionalityData.summary.total + validationData.summary.total;
    ...
    console.log(
      `✅ Core Functionality: ${functionalityData.summary.passed}/${functionalityData.summary.total} (${functionalityData.summary.successRate})`,
    );
    console.log(
      `🔍 Input Validation: ${validationData.summary.passed}/${validationData.summary.total} (${validationData.summary.successRate})`,
    );
    ...
    return NextResponse.json({
      ...
      conclusion: {
        playerProfileStatus:
          totalFailed === 0 ? "FULLY FUNCTIONAL" : "HAS ISSUES",
