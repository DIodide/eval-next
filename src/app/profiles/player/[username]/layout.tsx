...
export async function generateMetadata({ params }: PlayerLayoutProps): Promise<Metadata> {
  ...
  try {
    const player = await api.playerProfile.getPublicProfile({ username: resolvedParams.username });
    ...
    if (player.main_game?.name) descriptionParts.push(`specializing in ${player.main_game.name}`);
    if (player.school ?? player.school_ref?.name) descriptionParts.push(`from ${player.school ?? player.school_ref?.name}`);
    if (player.class_year) descriptionParts.push(`graduating ${player.class_year}`);
    ...
    return {
      ...
      twitter: {
        ...
        title,
        description,
        images: player.image_url ? [player.image_url] : undefined,
      },
    };
  } catch (error) {
    ...
    return {
      ...
      description: 'View player profile on EVAL - the college esports recruiting platform.',
    };
  }
}
