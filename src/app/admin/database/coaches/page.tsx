  ...
  return (
    <div className="space-y-8">
      ...
      <Card className="bg-gray-800 border-gray-700">
        ...
        <CardContent>
          <form action="/admin/database/coaches" method="get" className="flex space-x-4">
        ...
        {coaches.length === 0 ? (
          <Card className="bg-gray-800 border-gray-700">
            <CardContent className="pt-6 text-center">
              ...
              <h3 className="text-lg font-medium text-white mb-2">No coaches found</h3>
              <p className="text-gray-400">
                {searchTerm ? 'Try adjusting your search criteria.' : 'No coaches are currently registered.'}
              </p>
            </CardContent>
          </Card>
        ) : (
          coaches.map((coach) => (
            <Card key={coach.id} className="bg-gray-800 border-gray-700">
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right text-sm text-gray-400">
                    ...
                    {coach.updated_at && coach.updated_at !== coach.created_at && (
                      <div>
                        Updated: {new Date(coach.updated_at).toLocaleDateString()}
                      </div>
                    )}
                    <div className="flex items-center space-x-2 mt-2">
                      ...
                      {coach.school_id && (
                        <Button variant="outline" size="sm" asChild>
                          <a href={`/profiles/school/${coach.school_id}`} target="_blank">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-700">
                    <div className="grid gap-2 text-sm">
                      <div className="text-gray-400">
                        <span className="font-medium">Institution:</span> {coach.school}
                      </div>
                      {coach.school_id && (
                        <div className="text-gray-400">
                          <span className="font-medium">School ID:</span> {coach.school_id}
                        </div>
                      )}
                    </div>
                  </div>
