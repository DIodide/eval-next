...
const schoolAssociationRequestSchema = z
  .object({
    school_id: z.string().uuid().optional(),
  ...
  .refine(
    (data) => {
      ...
      if (data.is_new_school_request) {
        return (
          data.proposed_school_name &&
          data.proposed_school_type &&
          data.proposed_school_location &&
          data.proposed_school_state
        );
      } else {
    ...
    {
      message:
        "Either provide school_id for existing school or complete new school information",
    },
  );
...
export const coachProfileRouter = createTRPCRouter({
  ...
  getBasicProfile: coachProcedure.query(async ({ ctx }) => {
    const userId = ctx.auth.userId!; // Safe to use ! because coachProcedure ensures userId exists
  ...
  getSchoolInfo: coachProcedure.query(async ({ ctx }) => {
    const userId = ctx.auth.userId!; // Safe to use ! because coachProcedure ensures userId exists
  ...
  removeSchoolAssociation: onboardedCoachProcedure.mutation(async ({ ctx }) => {
    const userId = ctx.auth.userId!; // Safe to use ! because coachProcedure ensures userId exists
    ...
    try {
      ...
      try {
        ...
        console.log(
          `Coach ${userId} onboarded status set to false after removing school association`,
        );
      } catch (clerkError) {
  ...
  getRecentActivity: onboardedCoachProcedure.query(async ({ ctx }) => {
    const coachId = ctx.coachId; // Available from onboardedCoachProcedure context

    try {
      // Get recent activities in parallel
      const [recentRegistrations, recentMessages, recentFavorites] =
        await Promise.all([
  ...
  getAchievements: coachProcedure.query(async ({ ctx }) => {
    const coachId = ctx.coachId; // Available from coachProcedure context
