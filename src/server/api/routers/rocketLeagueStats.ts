...
const RocketLeagueStatsSchema = z.object({
  ...
  percent_offense: z.number().describe("Percent of time in offensive third (0-100)"),
  percent_defense: z.number().describe("Percent of time in defensive third (0-100)"),
  percent_neutral: z.number().describe("Percent of time in neutral third (0-100)"),
  first_man_percent: z.number().nullable().describe("Percent of time as first man (0-100)"),
  second_man_percent: z.number().nullable().describe("Percent of time as second man (0-100, 3v3 only)"),
  last_man_percent: z.number().nullable().describe("Percent of time as last man (0-100)"),
  ...
  boost_75_100: z.number().describe("Percent of time with 75-100% boost (0-100)"),
  ...
  small_to_big_ratio: z.number().describe("Small boost to big boost collection ratio"),
  stolen_to_collected_ratio: z.number().describe("Boost stolen to collected ratio"),
...
const RocketLeagueAllPlaylistsResponseSchema = z.object({
  ...
  duels: RocketLeagueStatsSchema.nullable().describe("Statistics for ranked duels playlist"),
  doubles: RocketLeagueStatsSchema.nullable().describe("Statistics for ranked doubles playlist"),
  standard: RocketLeagueStatsSchema.nullable().describe("Statistics for ranked standard playlist"),
  weighted_eval_score: z.number().describe("Weighted average eval score across all playlists"),
  playlist_scores: z.record(z.number()).describe("Individual eval scores by playlist"),
...
export type RocketLeagueAllPlaylistsResponse = z.infer<typeof RocketLeagueAllPlaylistsResponseSchema>;
  ...
  getAllPlayerStats: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        ...
        const apiUrl = new URL(`${EVAL_API_BASE}/rocket-league/player/all-stats`);
        ...
        console.log(`[RL Stats] Successfully validated response for player:`, validatedResponse.username);
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "An unexpected error occurred while fetching Rocket League stats"
        });
      }
    })
}); 