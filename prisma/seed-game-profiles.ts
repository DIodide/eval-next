...
async function main() {
  ...
  games.forEach(game => console.log(`   • ${game.name} (${game.short_name})`))
  ...
  for (const username of PLAYER_USERNAMES) {
    ...
    const existingGameIds = player.game_profiles.map(profile => profile.game_id)
    console.log(`   Existing profiles: ${player.game_profiles.map(p => p.game.short_name).join(', ') || 'None'}`)
    ...
    const numNewProfiles = Math.min(Math.floor(Math.random() * 3) + 1, availableGames.length)
    const selectedGames = getRandomElements(availableGames, numNewProfiles)

    console.log(`   Creating ${numNewProfiles} new profile(s) for: ${selectedGames.map(g => g.short_name).join(', ')}`)
    ...
    for (const game of selectedGames) {
      try {
        ...
        const leagueScore = Math.max(0, Math.min(100,
          baseScore + (Math.random() - 0.5) * variance * 2
        ))
        
        const combineScore = Math.max(0, Math.min(100,
          baseScore + (Math.random() - 0.5) * variance * 2
        ))
        ...
        console.log(`   ✅ Created ${game.short_name} profile - Rank: ${gameProfile.rank}, Role: ${gameProfile.role}`)
        console.log(`      Scores: League ${gameProfile.league_score}, Combine ${gameProfile.combine_score}, Rating ${gameProfile.rating}`)
      } catch (error) {
        console.log(`   ⚠️  Could not create ${game.short_name} profile:`, error)
      }
    }
  }
  ...
  for (const username of PLAYER_USERNAMES) {
    ...
    if (player) {
      console.log(`   ${username}: ${player.game_profiles.length} profile(s) - ${player.game_profiles.map(p => p.game.short_name).join(', ')}`)
    }
  }
}
